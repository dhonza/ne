 ...
 */
public class ATTreeDistanceWeighted implements IDistance<ATTree> {
    private double c1;

    public ATTreeDistanceWeighted(ParameterCombination parameters) {
        c1 = parameters.getDouble("GPAT.DISTANCE_WEIGHTED_C1");
    }
    ...
    private double distanceRecursive(ATNode a, ATNode b) {
        ...
        for (int i = 0; i < shorter; i++) {
            distance += distanceRecursive(a.getChild(i), b.getChild(i)) + c1 * deltas[i];
        }
        distance += rest;
