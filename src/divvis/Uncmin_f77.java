...
public class Uncmin_f77 extends Object {
     ...
     *                 evaluated at x.
     *                 2.) a method, gradientCD, that has the form
     *                 <p/>
     *                 public static void gradientCD(double x[],
     *                 double g[])
     *                 <p/>
     *                 where g is the gradientCD of f evaluated at x.  This
     ...
     * @param gpls     The gradientCD at the local minimum xpls
     * @param itrmcd   Termination code
     *                 ITRMCD =  0:  Optimal solution found
     *                 ITRMCD =  1:  Terminated with gradientCD small,
...
*/
        ...
        if (itrmcd[1] == 1) {
            ...
            //                 "probably converged, gradientCD small\n\n");
...
//                          "possibly unbounded\n\n");
     ...
     * @param iagflg   = 0 if an analytic gradientCD is not supplied
     * @param iahflg   = 0 if an analytic Hessian is not supplied
     * @param dlt      Trust region radius
     * @param gradtl   Tolerance at which the gradientCD is considered close enough
...
// SET FLAGS
    ...
    /**
     * <p/>
     * The dogdrv_f77 method finds the next Newton minimize (xpls) by the double dogleg
     * method.  It drives dogstp_f77.
     * <p/>
     * Translated by Steve Verrill, April 15, 1998.
     *
     * @param n        Dimension of the problem
     * @param x        The old minimize
     * @param f        Function value at the old minimize
     * @param g        Gradient or approximation at the old minimize
     * @param a        Cholesky decomposition of Hessian
     *                 in lower triangular part and diagonal
     * @param p        Newton step
     * @param xpls     The new minimize
     * @param fpls     Function value at the new minimize
     * @param minclass A class that implements the Uncmin_methods
     *                 interface (see the definition in
     *                 Uncmin_methods.java).  See UncminTest_f77.java for an
     *                 example of such a class.  The class must define:
...
// SOLVE LX=B. (FORWARD SOLVE)
     ...
     * gradientCD of the function to be minimized.
     * <p/>
     * Translated by Steve Verrill, April 21, 1998.
     *
     * @param n        The dimension of the problem
     * @param x        The point at which the gradientCD is to be approximated
     * @param minclass A class that implements the Uncmin_methods
     *                 interface (see the definition in
     *                 Uncmin_methods.java).  See UncminTest_f77.java for an
     *                 example of such a class.  The class must define:
     ...
     * @param g        A central difference approximation to the gradientCD
     */
...
// UNIT VECTOR, AND EVALUATE I-TH  COMPONENT OF GRADIENT.
     ...
     *                 should only fill the lower triangle (and diagonal)
     *                 of h.
     * @param fpls     fpls[1] -- fpls[n] contains the gradientCD
...
*/
        ...
        for (j = 1; j <= n; j++) {
            ...
            xtmpj = xpls[j];
            xpls[j] = xtmpj + stepsz;
            //System.out.println("calling gradientCD!!!");
     ...
     *                 function to minimize at the new minimize
     * @param g        finite difference approximation to the gradientCD
     * @param sx       Scaling vector for x
     * @param rnoise   Relative noise in the function to be minimized
     */
...
Java method is only used for gradientCD calculations.
...
// gradientCD
    ...
    /**
     * <p/>
     * The grdchk_f77 method checks the analytic gradientCD supplied
     * by the user.
     * <p/>
     * Translated by Steve Verrill, April 22, 1998.
     *
     * @param n        The dimension of the problem
     * @param x        The location at which the gradientCD is to be checked
     * @param minclass A class that implements the Uncmin_methods
     *                 interface (see the definition in
     *                 Uncmin_methods.java).  See UncminTest_f77.java for an
     *                 example of such a class.  The class must define:
     ...
     * @param gest     Finite difference gradientCD
     */
...
// ANALYTIC GRADIENT.
        ...
        System.out.print(" of the gradientCD method.\n\n\n");
     ...
     * @param analtl   Tolerance for comparison of estimated and
     *                 analytic gradients
     * @param iagflg   = 1 if an analytic gradientCD is supplied
...
// APPROXIMATION.
    ...
    /**
     * <p/>
     * The hookdr_f77 method finds a next Newton minimize (xpls) by the More-Hebdon
     ...
     * @param n        The dimension of the problem
     * @param x        The old minimize
     * @param f        The function value at the old minimize
     * @param g        Gradient or approximation at old minimize
...
// CHECK NEW POINT AND UPDATE TRUST REGION
     ...
     *
     * @param n      The dimension of the problem
     * @param g      The gradientCD at the current minimize
...
// BACK SOLVE, RESULT IN X
    ...
    /**
     * <p/>
     * The minimize method finds a next Newton minimize by line search.
     * <p/>
     * Translated by Steve Verrill, May 15, 1998.
     *
     * @param n        The dimension of the problem
     * @param x        Old minimize
     * @param f        Function value at old minimize
     * @param g        Gradient or approximation at old minimize
     * @param p        Non-zero Newton step
     * @param xpls     New minimize
     * @param fpls     Function value at new minimize
     * @param minclass A class that implements the Uncmin_methods
     *                 interface (see the definition in
     *                 Uncmin_methods.java).  See UncminTest_f77.java for an
     *                 example of such a class.  The class must define:
...
*/
     ...
     * @param gradtl Tolerance at which the gradientCD is considered
...
// IF NOT, SET THEM TO DEFAULT VALUES.
        ...
        if ((msg[1] / 2) % 2 == 1 && iagflg[1] == 0) {
            ...
            // System.out.print("\n\nOPTCHK   User requests that analytic gradientCD");
            // System.out.print(" be accepted as properly coded,\n");
            // System.out.print("OPTCHK   msg = " + msg + ",\n");
            // System.out.print("OPTCHK   but an analytic gradientCD is not");
...
// PRINT RESULTS
     ...
     * @param xpls   New minimize
     * @param fpls   Function value at new minimize
     * @param gpls   Gradient or approximation at new minimize
     * @param x      Old minimize
     * @param itncnt Current iteration
     * @param icscmx Number of consecutive steps >= stepmx
     *               (retain between successive calls)
     * @param itrmcd Termination code
     * @param gradtl Tolerance at which the relative gradientCD is considered
     *               close enough to zero to terminate the algorithm
     * @param steptl Relative step size at which successive iterates
     *               are considered close enough to terminate the algorithm
     * @param sx     Scaling vector for x
...
// CHECK WHETHER WITHIN TOLERANCE
            ...
            if (rgx <= gradtl[1]) {
                ...
                if ((msg[1] / 8) % 2 == 0) {
                    ...
                    // System.out.print("\n\nOPTSTP    The relative gradientCD is close");
                    // System.out.print(" to zero.\n");
                    // System.out.print("OPTSTP    The current minimize is probably");
...
*/
     ...
     * @param xpls   New minimize
     * @param gpls   Gradient or approximation at the new minimize
     * @param epsm   Machine epsilon
     * @param itncnt Iteration count
     * @param rnf    Relative noise in optimization function f_to_minimize
     * @param iagflg 1 if an analytic gradientCD is supplied, 0 otherwise
...
// TRIANGULAR PART.
     ...
     * @param n      The dimension of the problem
     * @param x      The old minimize
     * @param g      The gradientCD or an approximation at the old minimize
     * @param a      On entry: Approximate Hessian at the old minimize
     *               in the upper triangular part (and udiag)
     *               On exit:  Updated approximate Hessian at the new
     *               minimize in the lower triangular part and
     *               diagonal
...
// BFGS UPDATE
     ...
     * the gradientCD or the Hessian, and the optimization function
...
// CALCULATE SUB-DIAGONAL ELEMENTS OF COLUMN
     ...
     * minimize and update the trust region dlt.
     * <p/>
     * Translated by Steve Verrill, May 11, 1998.
     *
     * @param n        The dimension of the problem
...
C                    =3 F(XPLS) SUFFICIENTLY SMALL, BUT DRATIC MODEL
