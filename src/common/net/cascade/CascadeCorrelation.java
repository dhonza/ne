...
public class CascadeCorrelation implements ILearningAlgorithm, Serializable {
    ...
    public CascadeCorrelation(NeuralNetwork neuralNetwork, int maxLayersNumber, double acceptableError, int candNumber,
        ...
        this.outputsNumber = this.neuralNetwork.getOutputLayer().size();
    ...
    public void trainNetwork(TrainingSet trainingSet) throws Exception {
        ...
        while (this.neuralNetwork.getNumberOfLayers() <= this.maxLayersNumber);
        //double [][] outputs = this.neuralNetwork.getOutputValues(trainingSet, this.neuralNetwork.outputLayer().size());
    ...
    private int trainOutputs(TrainingSet trainingSet, ILearningAlgorithm learningAlgorithm, ISlopeCalcFunction slopeCalcFunction) throws Exception {
        ...
        try {
            do {
                ...
                Iterator<Neuron> neuronIterator = this.neuralNetwork.getOutputLayer().neuronList().iterator();
    ...
    private NeuronLayer includeCandidateNodes(int number) throws Exception {
        try {
            ...
            int hiddenLayerSize = this.neuralNetwork.getHiddenLayers().size();
            if (hiddenLayerSize > 0) {
                NeuronLayer lastHiddenLayer = this.neuralNetwork.getHiddenLayers().get(hiddenLayerSize - 1);
                this.neuralNetwork.fullyConnectLayers(lastHiddenLayer, candidateLayer, true);
            }
            NeuronLayer inputLayer = this.neuralNetwork.getInputLayer();
            this.neuralNetwork.fullyConnectLayers(inputLayer, candidateLayer, true);
            this.neuralNetwork.getLayers().add(this.neuralNetwork.getLayers().size() - 1, candidateLayer);
    ...
    private void integrateCandidate(NeuronLayer candidateLayer) throws Exception {
        try {
            this.neuralNetwork.fullyConnectLayers(candidateLayer, this.neuralNetwork.getOutputLayer(), true);
        }
        catch (Exception ex) {
    ...
    public DataForCorrelationComp getDataForCorrelation(int candidatesNumber, int outputsNumber, TrainingSet trainingSet, NeuronLayer candidatesLayer) throws Exception {
        try {
            ...
            while (patternIterator.hasNext()) {
                ...
                neuronIterator = this.neuralNetwork.getOutputLayer().neuronList().iterator();
