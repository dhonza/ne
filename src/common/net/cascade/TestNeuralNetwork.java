...
public class TestNeuralNetwork {
    ...
    public void testXor2() {
        ...
        try {
            ...
            network.fullyConnectLayers(network.getLayers().get(0), network.getLayers().get(1), true);
            network.fullyConnectLayers(network.getLayers().get(1), network.getLayers().get(2), true);
            network.fullyConnectLayers(network.getLayers().get(0), network.getLayers().get(2), true);
            ...
            info.mode = TrainMode.minimize;
            info.neuralNetwork = network;
            info.synapsesToTrain = network.getSynapses();
            network.printNetworkToFile("network.txt", true);
            while (true) {
                for (int i = 0; i < 10; i++) {
                    network.printNetworkToFile("newtest.txt", true);
                    rprop.train(trainingSet, info, new SlopeCalcFunctionQuickProp());
                    //quick.train(trainingSet, info, new SlopeCalcFunctionQuickProp());
