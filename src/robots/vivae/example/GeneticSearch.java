...
import robots.vivae.fitness.AverageSpeed;
import robots.vivae.fitness.CombinedFitness;
import robots.vivae.fitness.Damage;
import robots.vivae.fitness.FitnessFunction;
import robots.vivae.fitness.MovablesOnTop;
import robots.vivae.util.Util;
...
public class GeneticSearch {
	...
	public GeneticSearch(String scenario, double mutationRate, double crossoverRate, int generationCount, int populationSize) {
	...
	public static void main(String[] args) {
		final String scenario = "cfg/vivae/scenarios/arena3.svg";
...
//				int point = (int) Math.random()*(d1[i].length - 2) + 1;
...
//					 double max = Math.max(temp, d2[i][j]);
//					 double min = Math.min(temp, d2[i][j]);
//					 d1[i][j] = Math.random()*(max-min)+min;
//					 d2[i][j] = Math.random()*(max-min)+min;
				}
			}
		}
	...
	private WeightedNetwork roulette(double totalFitness,
		...
		for (WeightedNetwork net : nets) {
			...
			if (fitnessSoFar >= slice) {
				return new WeightedNetwork(0, copy(net.getNet()), i, net
						.getGenotype());
			}
			i++;
		}
